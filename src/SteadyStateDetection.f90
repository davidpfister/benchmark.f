!> @file
!! @defgroup group_ssd Steady_State
...
contains
    ...
    !> Compute the probability that a sample `x` reached steady-state.
    ...
    real(r8) function student(dof, a) result(t)
        ...
        f(x) = (1 + x**2 / real(dof, r8))**(-(dof + 1) / 2.0_r8) * gamma((dof + 1) / 2.0_r8) / sqrt(3.14_r8 * dof) / gamma(dof / 2.0_r8)
