module fee
	...
	character,           dimension(:), allocatable, private :: opaddsub			!Operador
	integer,										private	:: isaddsub = 1
	character,           dimension(:), allocatable, private :: opmuldiv			!Operador
...
contains
	...
	funclen = len_trim(tmp)
	do i = 1, funclen
		k = index(upper,tmp(i:i))
		if ( k /= 0) then
			tmp(i:i) = lower(k:k)
		end if
		k = index(tmp,'**')
	...
	call blanks(tmp)
	call recog_variables (tmp, variablenames)
	...
	do while (trim(toke) == '+' .or. trim(toke) == '-')
        if (isaddsub > size(opaddsub)) then
            opaddsub = [opaddsub, trim(toke)]
        else
	...
	do while (trim(toke) == '*' .or. trim(toke) == '/')
        if (ismuldiv > size(opmuldiv)) then
            opmuldiv = [opmuldiv(:), trim(toke)]
        else
	...
	if (allocated(opaddsub)) then
		deallocate(opaddsub)
	end if
	if (allocated(opmuldiv)) then
		deallocate(opmuldiv)
	end if
